...
- **فحص الموقع الشامل** — أداة تفحص كل صفحات الموقع تلقائياً (قايمة الصفحات ديناميكية من GitHub، مش ثابتة بالكود) وتقيّم الأداء وSEO وإمكانية الوصول وأفضل الممارسات والصور والروابط الداخلية وUI/UX، مع فحص مرئي اختياري بيحمّل الصفحة فعليًا ويحاكي شاشة موبايل، وتصدير النتيجة PDF
...
- **نظام شهادات تلقائي** — رفع قالب شهادة وتحديد أماكن الاسم/المادة/التاريخ عليه، ثم إصدار فردي أو جماعي حسب شرط استحقاق (نسبة حضور/درجات)
...
## 🔒 ملاحظات أمان (بعد المراجعة)

راجعت الريبو بالكامل بحثًا عن أي سر (API secret keys، كلمات سر، توكنز) متسرب بالغلط، والخلاصة:

- **مفيش أي سر حقيقي متسرب.** الـ`apiKey` الموجود في `js/config.js` (و`sw.js` و`firebase-messaging-sw.js`) هو **Firebase Web API Key** — ده مصمم أصلاً يكون عام وظاهر في أي كود جافاسكريبت شغال في المتصفح (زي أي موقع Firebase تاني)، ومش بيدّي صلاحيات لوحده. الحماية الفعلية بتيجي من **Firestore Security Rules** (مُدارة من Firebase Console مباشرة، مش موجودة كملف في الريبو ده) — يستاهل تتأكدي إنها مضبوطة صح حسب الأدوار (كل دور يقدر يقرا/يكتب على اللي يخصه بس)، لأنها هي خط الدفاع الحقيقي مش الـ`apiKey`.
- **مفتاح الخدمة (`FIREBASE_SERVICE_ACCOUNT`)** — زي ما موضح تحت، متخزن كـ GitHub Secret مش متسرب في الكود، تمام.
- **Cloudinary** — اسم الحساب (`CLOUD_NAME`) واسم الـ upload preset (`UPLOAD_PRESET`) ظاهرين في الكود (`js/cloud-upload.js` وغيره). ده طبيعي لرفع الملفات من المتصفح مباشرة (Unsigned Upload)، بس معناه إن أي حد شاف الريبو يقدر يعرف اسم الـpreset. يفضّل تتأكدي من إعدادات الـpreset على لوحة Cloudinary نفسها (مش من هنا) إن فيه حد أقصى لحجم الملف ونوعه مسموح بيه، عشان محدش يستغله لرفع ملفات كبيرة أو غريبة.
- مفيش أي باسورد أو GitHub token متسرب في أي ملف بالريبو.
